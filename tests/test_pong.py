...
from core.environment import criar_ambiente
from core.evaluate import jogar_partida
...
class EnvironmentTest(unittest.TestCase):
    @patch("core.environment.pong_v3.env")
...
class AgentsTest(unittest.TestCase):
    ...
    @patch("agents.catalog.os.path.exists", return_value=False)
...
class CheckpointsTest(unittest.TestCase):
    ...
    def test_falha_na_serializacao_nao_cria_checkpoint_parcial(self):
        with tempfile.TemporaryDirectory() as diretorio:
            ...
            with patch("agents.rl_agent.np.savez_compressed", side_effect=OSError("falha")):
...
class TrainTest(unittest.TestCase):
    @patch("train.train_rl.PARTIDAS_VALIDACAO", 3)
    @patch("train.train_rl.criar_ambiente")
    def test_avalia_rl_contra_baseline_sem_exploracao(self, criar_env):
        from train.train_rl import avaliar_para_selecao
...
class AvaliarRallyTest(unittest.TestCase):
    ...
    def _ambiente_com_observacoes_genetico(self, observacoes, recompensas=None):
        class AmbienteSequencial:
            def __init__(self, observacoes, recompensas):
                self.turnos = [
                    (nome, observacao)
                    for observacao in observacoes
                    for nome in ("second_0", "first_0")
                ]
                self.recompensas = recompensas or [0] * len(self.turnos)
                self.turno_atual = None
            ...
            def last(self):
                return (
                    self.turno_atual[1],
                    self.recompensas[self.indice_atual],
                    False,
                    False,
                    {},
                )
        ...
        return AmbienteSequencial(observacoes, recompensas)

    def test_reset_recebe_seed_explicita(self):
        from train import train_genetic
    ...
    def test_fitness_contabiliza_ponto_sofrido_antes_de_encerrar_rally(self):
        from train import train_genetic

        env = self._ambiente_com_observacoes_genetico(
            [self._observacao(0, 0)] * 2,
            recompensas=[0, 1, -1, 0],
        )
        agente = Mock()
        agente.escolher_acao.return_value = 1
...
class EvalAgenteTest(unittest.TestCase):
    @patch("train.train_genetic.avaliar_partida")
    @patch("train.train_genetic.criar_ambiente")
...
class ToolboxConfigTest(unittest.TestCase):
    ...
    def test_selecao_e_roleta_com_fitness_negativo(self):
        ...
        with patch("train.train_genetic.random.random", return_value=0.99):
